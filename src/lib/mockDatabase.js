...
const getUsers = () => {
  ...
  return JSON.parse(localStorage.getItem(KEYS.allUsers) || '{}');
};
const getDoctors = () => {
  if (typeof window === 'undefined') return {};
  return JSON.parse(localStorage.getItem(KEYS.allDoctors) || '{}');
};
const getRequests = () => {
  if (typeof window === 'undefined') return [];
  return JSON.parse(localStorage.getItem(KEYS.allRequests) || '[]');
};
