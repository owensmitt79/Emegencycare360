...
const PLANS = [
  {
    ...
    price: 5000,
    period: 'one-time',
    description: 'Basic medical listing & profile verification.',
    features: ['Directory Listing', 'Chat Consultation', 'Standard Profiles'],
    color: '#3b82f6',
  ...
  },
  {
    ...
    price: 10000,
    period: 'month',
    description: 'Full consulting features for active practices.',
    features: ['Unlimited Chats & Voice calls', 'HD Video Consultations', 'Priority Placement'],
    color: '#06b6d4',
  ...
  },
  {
    ...
    price: 80000,
    period: 'year',
    description: 'Save 33%. Best value for medical practitioners.',
    features: ['All Starter Features', 'Verified Badge Booster', '24/7 Priority Support Queue', 'Search Boost (3x)'],
    color: '#a855f7',
  ...
  },
  {
    ...
    price: 150000,
    period: 'one-time',
    description: 'Ultimate partnership tier. Never pay again.',
    features: ['Lifetime Access Guarantee', 'VIP Profile Status Indicator', 'Dedicated Account Executive', 'Featured Top Result Rank'],
    color: '#f59e0b',
    gradient: 'from-amber-500/20 to-orange-500/5',
  }
];

const DoctorPaymentGate = ({ doctor, onSuccess, onLogout, isRenewal = false }) => {
  const [selectedPlan, setSelectedPlan] = useState(PLANS[1]); // Default to 'monthly'
  ...
  const handlePay = async (e) => {
    ...
    try {
      ...
      const response = await apiClient.payDoctorRegistrationFee(selectedPlan.id);
      onSuccess(response.record, selectedPlan.name);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8" 
      style={{ background: 'linear-gradient(135deg, #090d16 0%, #0f172a 100%)' }}>
      
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        
        {/* Left Section: Plan Selector */}
        <div className="lg:col-span-2 space-y-6">
          <div className="text-left">
            <h1 className="text-3xl font-extrabold text-white tracking-tight sm:text-4xl">
              {isRenewal ? 'Renew Your Subscription' : 'Doctor Portal Payment Plan'}
            </h1>
            <p className="mt-2 text-slate-400 text-base max-w-xl">
              {isRenewal 
                ? 'Your subscription has expired. Please select a payment plan below to renew your access and resume consulting.'
                : 'Select a professional plan below to activate your medical practitioner profile and connect with emergency patients.'
              }
            </p>
          </div>
            ...
            {PLANS.map((plan) => {
              ...
              return (
                <div
                  ...
                  className={`relative rounded-3xl p-5 border cursor-pointer transition-all duration-300 flex flex-col justify-between overflow-hidden group select-none ${
                    isSelected 
                      ? 'border-transparent text-white ring-2 shadow-2xl' 
                      : 'border-slate-800 text-slate-300 hover:border-slate-700 hover:-translate-y-0.5'
                  }`}
                  style={{
                    background: isSelected 
                      ? `linear-gradient(135deg, ${plan.color}25, ${plan.color}0a)`
                      : 'rgba(255,255,255,0.02)',
                    boxShadow: isSelected ? `0 10px 30px -10px ${plan.color}33` : 'none',
                    borderColor: isSelected ? plan.color : undefined,
                  }}
                >
                  {plan.popular && (
                    <span className="absolute top-3 right-3 bg-cyan-500 text-slate-950 font-bold uppercase text-[9px] px-2 py-0.5 rounded-full tracking-widest">
                      Most Popular
                    </span>
                  )}
                    ...
                    <ul className="space-y-2 pt-2 border-t border-white/5 text-xs text-slate-300">
                      {plan.features.map((feat) => (
                        <li key={feat} className="flex items-center gap-2">
                          <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24" style={{ color: plan.color }}>
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    <span className="text-xs font-semibold group-hover:translate-x-1 transition-transform" style={{ color: isSelected ? plan.color : '#64748b' }}>
                      {isSelected ? 'Selected' : 'Select Plan'}
                    </span>
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                      isSelected ? 'border-transparent' : 'border-slate-700'
                    }`}
                    style={{ background: isSelected ? plan.color : 'transparent' }}>
                      {isSelected && (
                        <svg className="w-3 h-3 text-slate-950" fill="none" stroke="currentColor" strokeWidth={4} viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                      )}
                    </div>
        ...
        {/* Right Section: Secure Card Checkout */}
        <div className="space-y-6 lg:mt-16">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden text-white">
            ...
            <div className="text-left mb-6 relative z-10 border-b border-white/5 pb-4">
              ...
              <div className="mt-4 bg-slate-950 border border-slate-800 rounded-2xl p-4">
                ...
                <div className="flex justify-between items-center text-base mb-2 text-white font-bold">
                  <span>Due Today</span>
                  <span style={{ color: selectedPlan.color }}>₦{selectedPlan.price.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center text-[10px] text-slate-500 pt-2 border-t border-white/5">
                  <span>Billing Frequency</span>
                  <span>{selectedPlan.period === 'one-time' ? 'One-time charge' : `Renews every ${selectedPlan.period}`}</span>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handlePay} className="space-y-4 relative z-10">
              <div>
                <Label htmlFor="cardName" className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Cardholder Name</Label>
                <Input
                  id="cardName"
                  placeholder="Dr. Jane Doe"
                  value={cardData.name}
                  onChange={(e) => setCardData({ ...cardData, name: e.target.value })}
                  className="mt-1 text-white bg-slate-950 border-slate-850 focus:border-blue-500 focus:ring-blue-500/20"
                />
              </div>

              <div>
                <Label htmlFor="cardNumber" className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Card Number</Label>
                <div className="relative mt-1">
                  <Input
                    ...
                    className="pr-10 text-white bg-slate-950 border-slate-850 focus:border-blue-500 focus:ring-blue-500/20 font-mono"
                    maxLength={19}
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="expiry" className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Expiry Date</Label>
                  <Input
                    id="expiry"
                    placeholder="MM/YY"
                    value={cardData.expiry}
                    onChange={(e) => setCardData({ ...cardData, expiry: formatExpiry(e.target.value) })}
                    className="mt-1 text-white bg-slate-950 border-slate-855 focus:border-blue-500 focus:ring-blue-500/20 font-mono"
                    maxLength={5}
                  />
                </div>
                <div>
                  <Label htmlFor="cvv" className="text-slate-400 text-xs font-semibold uppercase tracking-wider">CVV</Label>
                  <Input
                    id="cvv"
                    placeholder="123"
                    value={cardData.cvv}
              ...
              <div className="pt-4 flex gap-3">
                <button
                  type="button"
                  onClick={onLogout}
                  className="flex-1 h-12 bg-slate-800 text-slate-300 font-semibold rounded-xl hover:bg-slate-700 hover:text-white transition-all text-sm border border-slate-700"
                  disabled={processing}
                >
                  Sign Out
                </button>
                <button
                  type="submit"
                  className="flex-1 h-12 text-white font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 text-sm"
                  style={{ 
                    background: `linear-gradient(135deg, ${selectedPlan.color}, ${selectedPlan.color}dd)`,
                    boxShadow: `0 8px 20px -8px ${selectedPlan.color}` 
                  }}
                  disabled={processing}
                >
                  {processing ? (
...
const planDetails = {
  basic: { name: 'Basic Registration', price: '₦5,000', period: 'one-time', color: '#3b82f6' },
  monthly: { name: 'Starter Monthly', price: '₦10,000', period: 'month', color: '#06b6d4' },
  annual: { name: 'Premium Annual', price: '₦80,000', period: 'year', color: '#a855f7' },
  lifetime: { name: 'Lifetime Elite', price: '₦150,000', period: 'one-time', color: '#f59e0b' },
};
...
const DoctorDashboardPage = () => {
  ...
  const handlePaymentSuccess = (updatedUserRecord, planName = 'basic') => {
    ...
    toast.success(`Subscription plan '${planName}' activated successfully!`);
  };
  ...
  const subscriptionPlan = doctorProfile.subscriptionPlan;
  const subscriptionExpiresAt = doctorProfile.subscriptionExpiresAt;
  
  const currentPlanId = doctorProfile.subscriptionPlan || 'basic';
  const currentPlan = planDetails[currentPlanId] || planDetails.basic;
  
  const isSubscriptionExpired = subscriptionExpiresAt && new Date(subscriptionExpiresAt) < new Date();
  const isSubscriptionActive = isFeePaid && !isSubscriptionExpired;

  // ── Registration Fee & Subscription Payment Screen ──────────────────────────
          ...
          {activeTab === 'billing' && (
            <div className="space-y-8 animate-fade-in text-white">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Active Plan Detail */}
                <div className="lg:col-span-2 rounded-2xl border border-white/5 p-6 space-y-6 relative overflow-hidden"
                  style={{ 
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="text-xs uppercase font-bold tracking-widest text-slate-500">Current Plan</span>
                      <h3 className="text-2xl font-bold text-white mt-1">{currentPlan.name}</h3>
                      <p className="text-xs text-slate-400 mt-1">Full consulting permissions on the Emergencycare360 network.</p>
                    </div>
                    <Badge color={currentPlan.color}>
                      Active
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-4 border-t border-white/5">
                    <div>
                      ...
                      <p className="text-base font-bold text-white mt-0.5">{currentPlan.price}<span className="text-xs text-slate-500 font-normal">/{currentPlan.period}</span></p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 uppercase tracking-wide">Next Invoice Date</p>
                      <p className="text-base font-bold text-white mt-0.5">
                        {doctorProfile.subscriptionExpiresAt 
                          ? new Date(doctorProfile.subscriptionExpiresAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                          : 'Never (Lifetime)'}
                      </p>
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                      <p className="text-xs text-slate-500 uppercase tracking-wide">Status</p>
                      <p className="text-base font-bold text-emerald-400 mt-0.5">Auto-Renewal Active</p>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex gap-3">
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl font-bold text-white text-xs transition-all flex items-center gap-2 hover:opacity-90"
                      style={{ background: `linear-gradient(135deg, ${currentPlan.color}, #0c1a30)` }}
                    >
                ...
                <div className="rounded-2xl border border-white/5 p-6 space-y-4"
                  style={{ background: 'rgba(255,255,255,0.03)' }}>
                  <h4 className="font-bold text-white text-sm">Tier Limits & Perks</h4>
                  <ul className="space-y-3 text-xs text-slate-300">
                    <li className="flex items-center gap-2">
                      ...
                      <span>Unlimited emergency consultations</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <svg className="w-4 h-4 text-emerald-400 shrink-0" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                      </svg>
                      <span>Chat, Voice & HD Video calls enabled</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <svg className="w-4 h-4 text-emerald-400 shrink-0" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                      </svg>
                      <span>HIPAA & NDPR data compliance</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <svg className="w-4 h-4 text-emerald-400 shrink-0" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                      </svg>
                      <span>Auto-settlement to registered bank details</span>
                    </li>
                  </ul>
                </div>
              ...
              <div className="rounded-2xl border border-white/5 overflow-hidden"
                ...
                <div className="px-6 py-4 border-b border-white/5">
                  <h3 className="font-bold text-white text-sm">Payment & Invoice History</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse text-white">
                    <thead>
                      <tr className="border-b border-white/5 text-slate-500 text-xs font-semibold uppercase tracking-wider bg-white/[0.01]">
                    ...
                    <tbody className="divide-y divide-white/5 text-sm text-slate-300">
                      <tr>
                        <td className="px-6 py-4 font-mono text-xs">INV-2026-9041</td>
                        <td className="px-6 py-4">May 31, 2026</td>
                        <td className="px-6 py-4">Subscription Plan Activation ({currentPlan.name})</td>
                        <td className="px-6 py-4 font-semibold text-white">{currentPlan.price}</td>
                        <td className="px-6 py-4">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-400/10 text-emerald-400">
                            Paid
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <button onClick={() => toast.success('Receipt download started (mocked).')} className="text-blue-400 hover:text-blue-300 font-semibold text-xs">
                            Download PDF
                          </button>
                        </td>
                      </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 backdrop-blur-md p-4 overflow-y-auto">
          <div className="relative w-full max-w-6xl my-8">
            <button
              ...
              className="absolute top-4 right-4 z-50 p-2 rounded-xl bg-slate-800 text-slate-400 hover:text-white transition-colors border border-slate-700"
            >
